; Orange Tohomon in water

	def_water_wildmons SHAMOUTI_ISLAND
	db 6 percent ; encounter rate
	wildmon LEVEL_FROM_BADGES - 2, GENGETSU
	wildmon LEVEL_FROM_BADGES + 0, GENGETSU
	wildmon LEVEL_FROM_BADGES + 2, GENGETSU
	end_water_wildmons

	def_water_wildmons BEAUTIFUL_BEACH
	db 6 percent ; encounter rate
	wildmon LEVEL_FROM_BADGES - 2, GENGETSU
	wildmon LEVEL_FROM_BADGES + 0, GENGETSU
	wildmon LEVEL_FROM_BADGES + 2, GENGETSU
	end_water_wildmons

	def_water_wildmons WARM_BEACH
	db 6 percent ; encounter rate
	wildmon LEVEL_FROM_BADGES - 2, FLANDRE
	wildmon LEVEL_FROM_BADGES + 0, FLANDRE
	wildmon LEVEL_FROM_BADGES + 2, GENGETSU
	end_water_wildmons

	def_water_wildmons SHAMOUTI_COAST
	db 6 percent ; encounter rate
	wildmon LEVEL_FROM_BADGES + 2, GENGETSU
	wildmon LEVEL_FROM_BADGES + 0, CPARSEE
	wildmon LEVEL_FROM_BADGES - 2, CYAMAME
	end_water_wildmons

	def_water_wildmons ROCKY_BEACH
	db 6 percent ; encounter rate
	wildmon LEVEL_FROM_BADGES - 2, GENGETSU
	wildmon LEVEL_FROM_BADGES + 0, GENGETSU
	wildmon LEVEL_FROM_BADGES + 2, GENGETSU
	end_water_wildmons

	def_water_wildmons SHAMOUTI_SHRINE_RUINS
	db 6 percent ; encounter rate
	wildmon LEVEL_FROM_BADGES - 2, GENGETSU
	wildmon LEVEL_FROM_BADGES + 0, GENGETSU
	wildmon LEVEL_FROM_BADGES + 2, GENGETSU
	end_water_wildmons

	def_water_wildmons NAVEL_ROCK_OUTSIDE
	db 6 percent ; encounter rate
	wildmon 80, GENGETSU
	wildmon 80, DLETTY
	wildmon 80, TEI
	end_water_wildmons

	db -1 ; end
